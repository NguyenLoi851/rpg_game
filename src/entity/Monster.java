...
public class Monster extends Enemy {
	...
	public Monster(int x, int y, Vector facing, int delayTime, int frameCount, byte imgID, byte attackID, float speed,
			Room room, int hp, boolean freeze, int damage, int attackTime) {
	...
	public void BFSMove() {
		...
		if (super.x % Tile.size == 0 && super.y % Tile.size == 0) {
			...
			facing = bfs.FindNextNode(super.y / Tile.size, super.x / Tile.size, player.y / Tile.size,
					player.x / Tile.size);
		}
		super.x = (int) (super.x + facing.x * speed);
		super.y = (int) (super.y + facing.y * speed);
