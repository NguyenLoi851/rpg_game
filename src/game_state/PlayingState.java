...
public class PlayingState extends GameState {
	...
	public PlayingState(GameStateManager gameStateManager) {
		...
		if (gameStateManager.blood_fall > 0) {
			...
			String[] sentences = new String[] { "Chuyện gì vừa xảy ra vậy ???", "Sao mình lại về chỗ này ???" };
			byte[] imgIDs = new byte[] { Resources.PLAYER, Resources.PLAYER };
			this.conversation = new Conversation(150, imgIDs, sentences);
		} else {
			this.inConversation = true;
			String[] sentences = new String[] { "Lũ Pales khốn kiếp", "Ta sẽ tiêu diệt các ngươi", "Giỏi vào mà ăn" };
			byte[] imgIDs = new byte[] { Resources.PLAYER, Resources.PLAYER, Resources.MONSTER };
			this.conversation = new Conversation(150, imgIDs, sentences);
		}
	}
				...
				if (p) {
					if (!GameOverMenu.result) {
						...
						if (player.x <= Tile.size * 4) {
							this.inConversation = true;
							String[] sentences = new String[] { "Sao ngươi không tấn công bọn ta",
									"Tôi không muốn ai phải đổ máu nữa", "Cuộc chiến này quá phi nghĩa",
									"Chúng ta làm hòa nhé", "Hi hi, mình đồng ý", "Mãi bên nhau bạn nhé <3" };
							byte[] imgIDs = new byte[] { Resources.AD, Resources.PLAYER, Resources.PLAYER,
									Resources.PLAYER, Resources.AD, Resources.AD };
							this.conversation = new Conversation(150, imgIDs, sentences);
	...
	public void Render(Graphics g) {
		...
		if (gameStateManager.blood_fall > 0 && world.GetCur() < World.count - 1) {
			if (gameStateManager.blood_fall <= 5)
				g.drawImage(Resources.TEXTURES.get(Resources.BLOOD_FALL), 0, gameStateManager.blood_fall * 60 - 300,
						WindowManager.WIDTH, WindowManager.HEIGHT, null);
			else
				g.drawImage(Resources.TEXTURES.get(Resources.BLOOD_FALL), 0, 0, WindowManager.WIDTH,
						WindowManager.HEIGHT, null);
		}
		if (this.inConversation) {
			if (conversation != null)
				conversation.Display(g);
		}
